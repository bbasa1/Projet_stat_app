...
try(data_merged <- data_merged[AGE - 2 >= age_min, ], silent=TRUE)
try(data_merged <- data_merged[AGE + 2 <= age_max, ], silent=TRUE)
try(data_merged <- data_merged[HHPRIV==1, ], silent=TRUE)
try(data_merged <- data_merged[HHLINK==1 | HHLINK==2, ], silent=TRUE)
...
# 
# 
# df_plot_copy <- copy(df_plot)
# df_plot_copy <- ff_interaction(df_plot_copy, Pays_label, Sexe)
# df_plot_copy
# 
# # Puis de tracé
# titre <- paste("Taux d'activité par pays,\n sexe et année d'enquête (entre", age_min, "et", age_max, "ans)", sep = " ")
# titre_save <- paste(pays, "Taux_activite_pays_sexe_annee_enqu.pdf", sep ='_')
# titre_save <- paste(repo_sorties, titre_save, sep ='/')
# x <-"Annee_enquete"
# sortby_x <- "Annee_enquete"
# y <- "tx_activite"
...
# xlabel <-"Année d'enquête"
# ylabel <-"Pays"
# ligne <- "Pays_label_Sexe"
# facet <- "Age_tranche"
# 
# 
# graph <- trace_point_lines(df_plot_copy, x, sortby_x, y, fill, xlabel, ylabel, titre, titre_save, ligne)
# # list_graph[[longueur_liste + 1]] <- graph
# # longueur_liste <- longueur_liste + 1
# # 
# # 
